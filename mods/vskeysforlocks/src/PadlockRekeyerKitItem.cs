...
{
    public class PadlockRekeyerKitItem : Item
    {
        ...
        public override void OnLoaded(ICoreAPI api)
        {
            ...
            interactions = ObjectCacheUtil.GetOrCreate(api, "padlockkeyerInteractions", () =>
            {
                ...
                return new WorldInteraction[]
                {
                    ...
                    {
                        ActionLangCode = "padlockrekeyerkit:heldhelp-padlockrekeyerkit",
                        HotKeyCode = "sneak",
            ...
            if (blockSel != null && byEntity.World.BlockAccessor.GetBlock(blockSel.Position).HasBehavior<BlockBehaviorLockable>(true))
            {
                ...
                if (!modBre.IsReinforced(blockSel.Position))
                {
                    if (api is ICoreClientAPI)
                        (api as ICoreClientAPI).TriggerIngameError(this, "incomplete", Lang.Get("padlockrekeyerkit:ingameerror-cannotuserekeyer-notreinforced"));
                    return;
                }
                ...
                {
                    if (api is ICoreClientAPI)
                        (api as ICoreClientAPI).TriggerIngameError(this, "incomplete", Lang.Get("padlockkey:ingameerror-cannotuserekeyer-notlocked"));
                    return;
                }
